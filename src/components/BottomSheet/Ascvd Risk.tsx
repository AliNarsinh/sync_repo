...
const AscvdRisk: React.FC<BinnahProps> = ({ data, setIsBottomSheetOpen }) => {
  ...
  const paraThreeText = translate(`risks_signs.${data?.title}.para_three`) || '';

  const { beforeList: paraTwoIntro, listItems: paraTwoList } = splitNumberedParagraph(paraTwoText);
  const { beforeList: paraThreeIntro, listItems: paraThreeList } = splitNumberedParagraph(paraThreeText);
      ...
      <div className="bg-white relative w-full">
          ...
          <div className="w-full relative">
            <div>
              ...
              <h1 className="font-inter font-bold sm:text-xl text-sm leading-[20.6px] text-center text-primary-color sm:w-full w-40 sm:m-0 m-auto">
                {translate('missing_general.over_all.Ascvd Risk')} {translate('missing_general.is')}{' '}
        ...
        <div className="container lg:w-3/5 sm:w-4/5 w-full mx-auto sm:px-0 px-3 ">
          <ScrollWithin>
            ...
            {paraThreeList.length > 0 && (
              <ol className="list-decimal pl-5 mb-3.5 text-sm text-dark-grey font-inter">
                {paraThreeList.map((item, index) => (
                  <li key={`paraThree-${index}`}>{item}</li>
                ))}
              </ol>
            )}
          </ScrollWithin>
